...
dirs = {
    ...
    "Videos": [".avi", ".mp4", ".gif"],
    "Images": [".bmp", ".png", ".jpg", ".PNG", ".JPEG", ".jpeg"],
    "Documents": [".txt", ".pptx", ".docx", ".csv", ".xls", ".odp", ".pages", ".pdf", ".xlsx", ".py", ".ics"],
    "Executable": [".exe"]
}

# dossier de tri
path_sale = Path.home() / "Downloads"
path_sale.mkdir(exist_ok=True)
path_tri = path_sale / "Tri"
path_tri.mkdir(exist_ok=True)

for cle, valeur in dirs.items():
    if path_tri.exists():
        path_all = path_tri / cle
        path_all.mkdir(exist_ok=True)
print(path_tri)

if path_tri.exists() and path_all.exists():
    files = [element for element in path_sale.iterdir() if element.is_file()]
    for file in files:
        extension = file.suffix
        for categorie, extensions in dirs.items():
            if extension in extensions:
                destination = path_tri / categorie

...
nombre_essai = 5
for i in range(1, 6):
    user = input("quel est le secret : ")
    essai = nombre_essai - i
    print(f"Il vous reste {essai} essai")
    if essai == 0:
        print(f"Il vous reste {essai} essai et le nombre mysterieux est : {valeur_myster}")
        break
"""
"""
import random
...
vie_policier = 100
vie_bandit = 200
choix_coup_police = {"petit_coup": 10, "moyen_coup": 20, "gros_coup": 30, "coup_tonnerre": 40, "coup_pistolet": 80}
seuil_critique_policier = 20
seuil_critique_bandit = 50
choix_coup_bandit = {"petit_coup": 10, "moyen_coup": 20, "coup_marteau": 50, "coup_chaine": 70}
print(range(1, len(choix_coup_police)))
while True:
    vie_bandit = 40
    ...
    if vie_bandit < seuil_critique_bandit:
        try:
            ...
            if user_bandit_resiste not in ['oui', 'non']:
                print("Choix inconnue")
        except ValueError:
            print("Vous avez un autre type d'erreur sûrement vous avez mis des chiffres")

        if user_bandit_resiste == 'oui':
            print("Le bandit se rend, fin de la partie !")
            break
        elif user_bandit_resiste == 'non':
            print("Le combat continue !")
            try:
                user_police = int(input("Policier choisir : "))
                if user_police not in range(1, len(choix_coup_police) + 1):
                    print("choix inconnu")
                if user_police == 1:
                    vie_bandit -= choix_coup_police["petit_coup"]
                    print(f"Il reste au bandit {vie_bandit}")
                    if vie_bandit <= 0:
                ...
                print("Error : Choix inconnu, veuillez choisir entre 1 à 5")
            continue
...
for tour in range(nombe_potion):
    essai = nombe_potion - tour
    user_police = input("Policier choisir un chiffre d'attaque ou 'P' pour rendre une potion de vies : ")
    if user_police.lower() == 'p':
        vie_policier += potion_policier
        print(f"Il vous reste {essai} potion et vous avez augmenté votre vies de {vie_policier}")
    if user_police == '1':
        vie_bandit -= choix_coup_police["petit_coup"]
...
"""
                coup_policier_choisi = coup_policier[user_police - 1]
